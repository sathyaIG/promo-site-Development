...
<div class="content-page">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body" style="position: relative;">
                                <a class="btn btn-outline-primary ms-auto" href="{{ admin_url('report') }}"
                            ...
                            <form id="updateForm" method="post"
                                action="{{ admin_url('create_group/' . encryptId($promotion->id)) }}">
                                @csrf
                                ...
                                <table id="region_list_table" class="table table-striped table-bordered"
                                    style="width: 97%;margin-left: 16px;">
                                    <thead>
                                        <tr>
                                            <th>S.No</th>
                                            <th>SKU ID</th>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($preview as $index => $listDetails)
                                            <tr>
                                                ...
                                                <td>{{ $listDetails->skuId ? $listDetails->skuId : '' }}
                                                </td>
                                            ...
                                            <td>{{ isset($listDetails->offerDetails) ? $listDetails->offerDetails : '' }}
                                                ...
                                                @if ($listDetails->approveRejectStatus == 2)
                                                    <td class="for"><input type="number" autocomplete="off" readonly
                                                            class="form-control label-box-rejected"
                                                            name="category_values[]"
                                                            id="category_value_{{ $index }}"
                                                            value="{{ $listDetails->category_value }}">
                                                        <div class="error-message" style="color: red"></div>
                                                        <input type="hidden" name="indexId[]"
                                                            id="indexId{{ $index }}"
                                                            value="{{ $listDetails->id }}">
                                                    </td>
                                                @else
                                                    <td class="for"><input type="number" min="0"
                                                            step="0.1" onkeydown="return event.keyCode !== 69"
                                                            autocomplete="off" class="form-control label-box"
                                                            name="category_values[]"
                                                            id="category_value_{{ $index }}"
                                                            value="{{ $listDetails->category_value }}">
                                                        <div class="error-message" style="color: red"></div>
                                                ...
                                                <td>
                                                    ...
                                                    @if ($listDetails->approveRejectStatus == 2)
                                                        <a class="reasoncheck" style="cursor: pointer;"
                                                            data-reason="{{ $listDetails->rejectComments }}">Already
                                                            Reject Click to View Reason</a>
                                                    @else
                                                        <a class="btn btn-xs btn-danger reject" id="reject"
                                                            data-id="{{ encryptId($listDetails->id) }}"
                                                            data-promotion="{{ encryptId($promotion->id) }}"><i
                                                                class="fa fa-ban"></i></a>
                                                    @endif
                                                </td>
                                            </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close cls-modal" data-bs-dismiss="modal" id="close_modal"
    ...
    <div id="reason_modal" class="modal fades" tabindex="-1" role="dialog" aria-labelledby="reason_modal"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="Upload_promotion_modalLabel">Reason</h4>
                    <button type="button" class="btn-close cls-modal" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
    ...
    @push('script')
        <script>
            ...
            $('#reject_submit').on('click', function(event) {
                ...
                $.ajax({
                    url: "{{ admin_url('Reject_sku_group') }}",
                    type: 'POST',
                    data: {
